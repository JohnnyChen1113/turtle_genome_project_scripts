...
topNum=25
threads=20
bwa_mem2=${PWD}/bwa-mem2-2.0pre2_x64-linux/bwa-mem2 
# Show the uasge
...
#bwa-mem2 index -p ${1%.*}/${1%.*} ${1%.*}/${1%.*}.sorted.fasta
else
...
then
  ...
  #bwa-meme mem -A 1 -B 4 -E 50 -L 0 -t 16 ${1%.*}/${1%.*} 1_R1.fastq.gz | samtools view -Shb - > ${1%.*}/1.bam
  #bwa-mem2 mem -A 1 -B 4 -E 50 -L 0 -t ${threads} ${1%.*}/${1%.*} 1_R1.fastq.gz | samtools view -Shb - > ${1%.*}/1.bam
else
...
then
  ...
  #bwa-meme mem -A 1 -B 4 -E 50 -L 0 -t 16 ${1%.*}/${1%.*} 1_R2.fastq.gz | samtools view -Shb - > ${1%.*}/2.bam
  #bwa-mem2 mem -A 1 -B 4 -E 50 -L 0 -t ${threads} ${1%.*}/${1%.*} 1_R2.fastq.gz | samtools view -Shb - > ${1%.*}/2.bam
else
...
cat ${1%.*}/${1%.*}.sorted.fasta | grep ">" | head -${topNum} | cut -d ' ' -f 1 | tr -d '>' | paste -s -d ' ' > ${1%.*}/logest${topNum}.id && \
